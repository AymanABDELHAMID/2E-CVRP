...
def create_model_1(depot, hubs, clients,
    ...
    model.addConstrs((gp.quicksum(z[n_c + h, int(c), r] for c in C for h in H) == robot[r] for r in R),
                 name="sameHub2")
    # new constraints for the decision variable "w"
    # (New1) - w == 1 if robot r serves client c from hub h - #TODO: add it to the formulation
    # model.addConstrs((x[c, r, h] == w[h, r] for c in C for r in R for h in H), name="w_constraint")

    # new constraint to robot
    # (New2) - robot[r] == 1 if it is used
    # probably not needed
